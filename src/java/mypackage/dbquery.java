...
{
    try {
        String str="select team.*,commando.name from commando,team,teammembers where commando.clogin_id=team.Clogin_id and team.Tid=teammembers.Tid and teammembers.Clogin_id='"+cid+"'";
        rs=st.executeQuery(str);
    } catch (Exception e) {
    }
    return rs;
   ...
   int i=0;
    try {
        ...
        if(rs.next())
        {
            ...
            sm.send(email, "YourPassword is"+password, "YourPassword");
            i=1;
        }
    } catch (Exception e) {
        System.err.println(e.toString());
